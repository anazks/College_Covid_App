...
function Register() {
    ...
    const submitted = async (e) => {
        ...
        try {
            ...
            await result.user.updateProfile({ displayName: "Student" });
            userObj.id = result.user.uid
            result = await addUsertoDB(userObj)
            console.log(result)
            navigate("/");
        } catch (error) {
    ...
    return (
        <div style={{
            ...
            <Card sx={{ maxWidth: 380, p: 3, minHeight: 1200, display: 'flex', flexDirection: 'column', justifyContent: "space-evenly" }}>
                ...
                <Button disabled={loading} variant="contained" onClick={submitted}>Register as Student</Button>
